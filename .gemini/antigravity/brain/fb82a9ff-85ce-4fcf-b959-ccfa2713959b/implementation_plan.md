...
- **Deliverables**:
    - [ ] Modify `GoogleSheetsService` to manage `dvdSpreadsheetId` and `vhsSpreadsheetId`.
    - [ ] Update `provisionUserSheet` to ensure both files exist (or create them).
...
- **Deliverables**:
    - [ ] Update `CollectionProvider` to expose the current `SpreadsheetID` based on format.
    - [ ] Update `LandingPage` to select the correct ID when clicking "DVD" or "VHS".

### Phase 8.3: First Genre Bootstrapping
- **Goal**: Enforce organized data entry for new collections.
- **Deliverables**:
    - [ ] Create `StartupGenreModal`.
    - [ ] Trigger logic: If `getGenres(currentFormat)` is empty -> Show Modal.
...
- **Deliverables**:
    - [ ] Create `GenreManagerModal` (Rename/Delete).
    - [ ] Implement `renameGenre` in Service.
    - [ ] Implement `deleteGenre` in Service (with "Delete all movies" warning).

### Phase 8.5: Manual Migration Support
- **Goal**: Ensure the app gracefully handles the transition.
- **Deliverables**:
    - [ ] Update `provisionUserSheet` to fallback: if "MoviePWA DVD Collection" is missing but "MoviePWA Collection" exists, use it (and maybe warn user/auto-rename if possible, or just treat as DVD).
...
- **Management**: Implement `renameGenre` and `deleteGenre`.
...
- **Create new component** (Do not touch `RandomMoviePicker`).
...
- Integrate `useGenres` to list current genres.
...
### Services
#### [MODIFY] [GoogleSheetsService.ts](file:///c:/Users/gabri/.gemini/antigravity/scratch/movie-pwa/src/services/GoogleSheetsService.ts)
- Implement `renameGenre(oldName, newName, format)`:
    - Find sheet by title.
    - Call `batchUpdate` with `updateSheetProperties` to change title.
- Implement `deleteGenre(genreName, format)`:
    - Find sheet by title.
    - Call `batchUpdate` with `deleteSheet` to remove the tab (and all its data).
...
- **Remove Barcode Scanner**:
    ...
    - Clean `MovieEditorModal.tsx` (remove "Scan" button and logic).
    - **Note**: Keep `barcode` field in `Movie` type as it is now used as a unique ID for Gemini matching.
...
- Update `Movie` type to include `backdropValue`.
...
- Add `tags: string[]` to `Movie` interface.
...
#### [MODIFY] [src/types/index.ts]
- Add `status?: 'watched' | 'watching' | 'plan_to_watch' | 'dropped'` to `Movie`.
...
- **Range**: Expand read/write ranges from `A:U` to `A:V`.
...
## Proposed Changes
  ...
  - **[NEW] Mixed Search (Movies + TV)**:
      ...
      - Filter results to only show `movie` and `tv` types.
      - Display "TV" badge in results list.
  - **[NEW] Support TV Series Links**:
      - Detect `/tv/` in URL.
      - Fetch from `/tv/{id}` endpoint.
      - Map `name` -> `title`, `first_air_date` -> `year`, `backdrop_path` -> `backdrop`.
...
#### [DONE] [x] **Critics Ratings Implementation**
  - [x] Create `OmdbService` or integrate into `MovieEditorModal`
  - [x] Add Rotten Tomatoes/Metacritic fields to `Movie` type and Sheets
...
### Automated Tests
- [x] browser: Open `MovieEditorModal`, verify OMDB fetch.
...
## Fase 18: Agrupamento Visual de Duplicatas (Visual Grouping)
- **Objetivo**: Reduzir a poluição visual combinando filmes repetidos em um único card, mantendo a integridade dos dados individuais.
- **Regras de Agrupamento**:
    - **Identificador**: Usa `tmdbId` (se disponível) ou combinação `Title` + `Year` (case-insensitive).
    - **Visualização**: Exibe o poster do *primeiro* item encontrado.
- **UI (Interface)**:
    - **Badge**: Adicionar contador (ex: "3") em um círculo destacado no canto do poster.
    - **Efeito**: (Opcional) CSS sutil parecendo "cartas empilhadas".
- **Interação (Regras de Negócio)**:
    - **Clique no Grupo**: Abre `DuplicateSelectionModal`.
    - **Modal de Seleção**: Lista as versões (diferenciando por: Nota, Data de Adição ou Formato se misturado).
    - **Edição**: Edita apenas a instância selecionada.
    - **Exclusão**:
        - Ao excluir uma instância, o grupo diminui (x3 -> x2).
        - Se restar apenas 1, o grupo deixa de existir visualmente.
        - **Proteção**: Confirmação padrão "Excluir esta cópia?".

### Arquivos Afetados
- `src/hooks/useMovieGrouping.ts` (Novo Hook)
- `src/components/modals/DuplicateSelectionModal.tsx` (Novo Componente)
- `src/pages/HomePage.tsx` (Implementar hook e lógica de clique)
- `src/components/movies/MovieCard.tsx` (Adicionar prop `groupCount` e badge)
